...
Dumps JSON to stdout, logs progess to stderr.

Example usage:
    python -m edx_import.github_to_json.py main 1> edx_import/export.json
Or just:
    make edx_import_do_export

Requirement installation:
    make edx_import_install  # in a virtualenv, from repo root

Other requirements:
    * Python >=3.8
    * Expects a personal GH access token at `edx_import/github_token`.
      (The access token only needs read access.)
    * Expects list of repo names for transfer at `edx_import/repos.txt`.

Linting:
    make edx_import_install_dev  # in a virtualenv, from repo root
    make edx_import_lint
"""
...
EDX_ADMIN_TEAM_SLUG = TeamSlug("edx-admin")
...
def main():
    ...
    repo_names = get_repo_names_to_move()
    gh_token = get_github_token()

    LOG.info(" Authenticating.")
    gh_client = gh_api.Github(gh_token)
    edx_org = gh_client.get_organization("edx")
    ...
    teams = fetch_teams(gh_headers, edx_org)
    repo_permissions = fetch_repo_permissions(gh_headers, edx_org, repo_names, teams)
...
def fetch_repo_permissions(
    ...
    * For each repo,
    ...
    for api_repo in sorted(api_repos, key=lambda api_repo: api_repo.name):
        ...
        # Handle edx admin team.
        # Recall from fetch_teams that this is a team we invented in order to
        # capture the users who currently have owner permissions on the edx org.
        # Members of this team get admin access to all imported repositories.
        team_access[EDX_ADMIN_TEAM_SLUG] = REPO_ADMIN
        for admin_username in team_members[EDX_ADMIN_TEAM_SLUG]:
...
def fetch_teams(gh_headers: dict, org: ApiOrganization) -> List[Team]:
    ...
    # We treat edx owners as team with admin permissions on all edx repos,
...
def get_github_token() -> str:
    ...
    """
    with open("edx_import/github_token") as token_file:
        return token_file.read().strip()


def get_repo_names_to_move() -> Set[RepoName]:
    """
    Load repo names from file.
    """
    with open("edx_import/repos.txt") as repos_file:
